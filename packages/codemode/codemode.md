...
This is the living design and status document for `@reddb-io/redcode-codemode` and its existing V2 OpenCode adapter.
...
`packages/redcode/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
