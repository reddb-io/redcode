...
2. Enter the **API URL**, such as `https://api.example.com/v1`. A missing `http://`, a trailing
   `/models` and a bare host (which gets `/v1`) are corrected for you.
...
that connection to its own id.

### What the wizard writes

Provider settings go to the global configuration file in `~/.red/code/` (the first that exists of
`config.jsonc`, `config.json`, `redcode.jsonc` and `redcode.json`; `redcode.json` is the common
choice when you create one by hand):
...
- **options.headers**: optional headers sent with every request, for example an organization or
  gateway header. Values may use `{env:VARIABLE}`. They are merged with, and can replace, the
  `Authorization` header built from the key.
