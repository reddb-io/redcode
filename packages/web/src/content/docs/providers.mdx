...
Redcode can use any **OpenAI-compatible** endpoint: vLLM, LM Studio, Ollama, LiteLLM, a company
gateway, or a hosted provider that is not in the `/connect` list.

:::tip
Most model servers and gateways offer an OpenAI-compatible API. If its documentation shows a base URL
ending in `/v1` and a `/v1/models` or `/v1/chat/completions` endpoint, it works here.
:::

### Connect with the wizard

1. Run `/connect` and choose **OpenAI-compatible**.
2. Enter the **API URL**, such as `https://api.example.com/v1`. A trailing `/models` and a bare host
   (which gets `/v1`) are corrected for you. Without a scheme, `https://` is used, except for local
   addresses (`localhost`, `*.local`, loopback, private `10.x`, `172.16-31.x`, `192.168.x` and `fc00::/7`
   addresses, and single-label names such as a container name), which get `http://`. Type `http://`
   yourself for a public host that only serves plain HTTP.
3. Confirm or change the **provider id**. It is suggested from the host (`api.together.xyz` becomes
   `together`) and is how the provider appears in configuration and model names (`together/<model>`).
   Each id is a separate provider, so you can connect several endpoints. Choosing the id of a
   configured provider updates it. Choosing the id of a built-in provider asks before overriding it:
   every model of that provider is then sent to this URL, and if you are logged in to it or saved a
   key for it, the wizard asks a second time because that login or key is replaced.
4. Confirm the **display name**.
5. Choose the **API type**: **Chat Completions** (`/chat/completions`, what most servers implement)
   or **Responses** (`/responses`, for OpenAI and servers that implement that API).
6. Enter the **API key**:
   - paste it, and it is saved in the credential store, not in configuration;
   - type `{env:MY_PROVIDER_KEY}`, and only that reference is saved in configuration. The value is read
     from the Redcode server's environment when requests are made and is never shown or written;
   - leave it empty for endpoints without a key, or to keep the key already saved for this provider
     and URL. A saved key is never reused for a different URL.
7. Redcode reads the model list from `<API URL>/models`. If the request fails or the list is empty,
   type the model ids instead, separated by commas, each optionally followed by its context size:
   `qwen2.5-coder 32k, llama3.1:8b 128000`.
8. The connection is saved and every instance reloads, so no restart is needed. A message names the
   configuration file that was written, and the model picker opens.

Cancelling before the last step saves nothing. The model list is fetched from the Redcode server, so
for `redcode attach` a `localhost` URL means the server's machine. The key and custom headers are not
sent when the model list redirects to another origin.

Run the wizard again for the same id to pick up new models. Models it added earlier that the endpoint
no longer lists are removed, unless you changed them with any setting besides `name` and `limit`.

**9Router** in the `/connect` list is a preset of this wizard, with its id, name and default URL
filled in. If your `9router` provider points to another endpoint, choosing **9Router** offers to move
that connection to its own id. The move updates `model`, `small_model`, agent and command models and
`enabled_providers`/`disabled_providers` in the global file; other configuration files that still
mention `9router` are listed, not edited.

### What the wizard writes

Provider settings go to the global configuration file in `~/.red/code/`: the first that exists of
`config.jsonc`, `config.json`, `redcode.jsonc`, `redcode.json`, `opencode.jsonc` and `opencode.json`,
or a new `config.jsonc`. `redcode.json` is the common choice when you create one by hand:

```jsonc title="~/.red/code/redcode.json"
{
  "provider": {
    "together": {
      "npm": "@ai-sdk/openai-compatible",
...
You can also write this by hand, globally or per project in `.red/code/redcode.json`. Project
configuration takes precedence over global configuration; if a project `apiKey`, `baseURL` or
provider filter (`enabled_providers`, `disabled_providers`) would stop the new connection from being
used, the wizard tells you.

- **npm**: the AI SDK package. `@ai-sdk/openai-compatible` calls `/chat/completions`. Use
  `@ai-sdk/openai` when the endpoint serves `/v1/responses`. For a mix under one provider, set
  `provider.npm` on individual models.
- **name**: display name in the UI.
- **options.baseURL**: the API URL, including `/v1` when the endpoint uses it.
- **options.apiKey**: optional. Prefer `{env:VARIABLE}` ([env vars](/docs/config#env-vars)) or a key
  saved with `/connect`; a key saved in the credential store is used when this is not set.
- **options.headers**: optional headers sent with every request, for example an organization or
  gateway header. Values may use `{env:VARIABLE}`, which the Redcode server resolves from its own
  environment. They can replace the `Authorization` header built from the key. Through the connect
  API, `Authorization`, `Proxy-Authorization`, `X-Api-Key` and `Api-Key` must use a reference, and saved
  headers are dropped when the URL changes, since they can carry credentials.
- **models**: the models to offer, keyed by the id the endpoint expects.

### Model limits

Each model's `limit` tells Redcode how much context it has, so it can compact a conversation before a
request is refused:

- **limit.context**: the most input and output a request may carry together.
- **limit.output**: the most tokens the model may produce in one request.
- **limit.input**: optional, when the provider caps input separately.

The wizard takes limits from the `/models` response when the endpoint reports them
(`context_length`, `max_context_length`, `context_window` or `max_input_tokens`, and output fields
such as `max_output_tokens`; OpenRouter-style `top_provider` limits are read too). Otherwise it uses
the [models.dev](https://models.dev) entry for the same model id, with router prefixes such as
`cc/` ignored, or the context size you typed. A model nobody describes gets a conservative guess: a
128,000-token context held back by 10% (115,200) and 8,192 output tokens.

If a provider refuses a request because it is larger than the model accepts, Redcode learns the limit
from the error and uses the smaller value from then on. `redcode debug limits` lists what was
learned. Setting or changing `limit.context` in configuration clears that lesson, so set it to what
the endpoint really enforces when you know it:
