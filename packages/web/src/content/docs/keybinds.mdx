...
Most terminals report `Alt+Enter` out of the box, either as an escape-prefixed return or through the kitty keyboard protocol. Two need a one-time change:

### macOS Terminal.app and iTerm2

Terminal.app sends a plain `Enter` for `Option+Enter` until you enable **Settings → Profiles → Keyboard → Use Option as Meta key**. In iTerm2, set **Settings → Profiles → Keys → Left Option key** (or Right) to **Esc+**. Kitty, WezTerm, Ghostty and Alacritty need no change.
