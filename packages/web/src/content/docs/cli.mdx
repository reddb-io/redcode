...
| <nobr><code>{"--tmp"}</code></nobr>         |       | Put session worktrees in the temporary directory                        |
...
Removes a provider completely: its saved key or login, its entry in the global config, and every global setting that uses it (default and small model, agent and command models, the enabled and disabled provider lists, and the reasoning models). It shows what will be cleared and asks for confirmation; pass `--yes` to skip the question.
...
| <nobr><code>{"--mdns-domain"}</code></nobr> | Custom mDNS domain name                    |
| <nobr><code>{"--cors"}</code></nobr>        | Additional browser origin(s) to allow CORS |
| <nobr><code>{"--tmp"}</code></nobr>         | Put session worktrees in the temporary dir |
...
Print the selected SQLite path or RedDB URL.
...
Show which database backend was selected and verify its connection.
...
Copy the current SQLite database to a remote RedDB. The source stays unchanged. The command records per-table counts and hashes, resumes completed tables, verifies the target, and writes a local completion manifest.

```bash
REDCODE_DATABASE_TOKEN=token redcode db migrate --to red://database.example:5050/redcode
```
...
| <nobr><code>{"--verbose"}</code></nobr>    |       | Trace the boot to stderr until the screen renders, then trace activity (provider requests, tool calls, compaction, guards) to the log at DEBUG. Also `REDCODE_VERBOSE=1`. Under the TUI the trace continues in `<data>/log/boot-<timestamp>.log`; `redcode run --verbose` prints it all to stderr |
...
| `REDCODE_AUTO_WORKTREE`               | boolean | Set to `0` to keep sessions out of worktrees          |
| `REDCODE_WORKTREE_LOCATION`           | string  | `repo` or `tmp`: where session worktrees go (`--tmp`) |
...
| `REDCODE_DATABASE_URL`                | string  | Use a remote RedDB for all durable Redcode state      |
| `REDCODE_DATABASE_TOKEN`              | string  | Authentication token for the configured RedDB         |
| `REDCODE_DB`                          | string  | Override the SQLite path when RedDB is not configured |
