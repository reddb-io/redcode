...
To store all durable Redcode state in a shared RedDB, set the URL in the global config:

```jsonc title="config.jsonc"
{
  "database": {
    "url": "red://database.example:5050/redcode",
  },
}
```

Pass credentials through `REDCODE_DATABASE_TOKEN`. `REDCODE_DATABASE_URL` overrides the global value. If neither is set, Redcode keeps using SQLite. A configured RedDB that cannot connect stops startup instead of silently opening SQLite.
...
In a Git repository, the first file write, edit, patch or non-read-only shell command that a writing session (such as `build`) makes in the primary checkout moves the session into its own worktree at `.red/worktrees/<name>`, on a new branch `<name>` based on `HEAD`. This happens with `--yolo` too: YOLO skips permission prompts, it does not skip isolation. A session that was already working in the primary checkout moves on its next write.

Uncommitted changes in the primary checkout are never moved, stashed or reset. They stay where they are, and the new worktree starts from `HEAD`. Plan, design and explore sessions, and directories outside Git, never get a worktree. In YOLO mode, a repository with no commits yet keeps the session in the primary checkout.

To keep sessions in the primary checkout, set `worktree.auto` to `false`, or set `REDCODE_AUTO_WORKTREE=0` in the environment. Either one turns automatic worktrees off in every mode.
...
To keep worktrees out of the repository, put them in the temporary directory: pass `--tmp` to `redcode`, `redcode run` or `redcode serve`, or set `worktree.location` to `"tmp"` in the global config or in the project's `.red/code/config.jsonc` (or `redcode.json`). They go to `<tmpdir>/redcode-worktrees/<repository>-<hash>/<name>`, on the same `<name>` branch. `worktree.tmpdir` replaces the system temporary directory. `--tmp` and `REDCODE_WORKTREE_LOCATION=tmp` take precedence over the config. The sidebar marks these worktrees `tmp`.

```jsonc title=".red/code/config.jsonc"
{
  "worktree": { "location": "tmp", "tmpdir": "/var/tmp" },
}
```

Temporary worktrees are disposable. `/worktrees` and `redcode worktrees` list them with the others, and `clean` removes the merged or stale ones and prunes those whose directory is gone, for example after a reboot. A worktree with uncommitted changes is never removed without an explicit force.
...
Built-in LSP servers are enabled by default. You can configure them through the `lsp` option or set it to `false` to disable them.
...
A turn is bounded in five places, so a session that stops making progress ends up saying so instead
of spinning. Every one of them accepts `false` to remove the bound entirely.

```json title="opencode.json"
{
  "$schema": "https://opencode.ai/config.json",
  "experimental": {
    "turn_stall": { "warn_ms": 120000, "abort_ms": 600000 },
    "tool_timeout": 600000,
    "loop_guard": { "correct_at": 3, "stop_at": 5, "nudge_at": 12 },
    "turn_steps": { "wrap_up_at": 198, "stop_at": 200 },
    "aux_timeout": 600000
  }
}
```
...
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `turn_stall`   | How long a turn may produce nothing before it is reported and, where nobody is watching, ended. Time a tool spends running or a permission spends awaiting an answer is not silence. In the TUI and desktop app the turn is reported rather than ended, since a person is there to read it.                                                                                            |
| `tool_timeout` | How long a single tool may run. Tools that carry their own deadline, wait for a person, or run a whole child turn are exempt, and time spent waiting on a permission prompt is not charged against it.                                                                                                                                                                                 |
| `loop_guard`   | How many identical tool calls in a row — same arguments, same result — before the model is told it is repeating itself, and before the turn ends. Identical calls that return different results are polling, and are left alone — up to `nudge_at`, where the repetition is mentioned once and the call still runs, so an answer carrying a timestamp cannot repeat forever unnoticed. |
| `turn_steps`   | How many steps a turn may run before the model is asked to stop and report what it did, and before the turn is stopped outright.                                                                                                                                                                                                                                                       |
| `aux_timeout`  | How long the calls around a turn — naming the session, compacting the conversation — may wait for a provider. Neither failure fails the turn.                                                                                                                                                                                                                                          |

To see whether any of them ever fires, and on what:

```bash
redcode debug guards
```

It prints a count per guard and action over the last week, loudest first, and the most recent
interventions. Thresholds are easier to argue about with that in hand than without it.
