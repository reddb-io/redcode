...
```json title="opencode.json"
{
  ...
  "experimental": {
    ...
    "tool_timeout": 600000,
    "loop_guard": { "correct_at": 3, "stop_at": 5 },
...
| `loop_guard` | How many identical tool calls in a row — same arguments, same result — before the model is told it is repeating itself, and before the turn ends. Identical calls that return different results are polling, and are left alone. |
