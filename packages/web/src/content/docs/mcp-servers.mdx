...
When the TUI is attached to a server on another host, the browser opens on the machine running the TUI, and after you approve it is redirected to `127.0.0.1` on that machine, where nothing is listening, so the page fails to load. Copy the full address from the browser's address bar and paste it into the sign-in dialog, then press <kbd>enter</kbd>. The TUI checks that the address belongs to the current attempt and sends the authorization code to the server, which finishes the flow. You can also forward the callback port instead, for example `ssh -L 19876:127.0.0.1:19876 server-host`, so the redirect reaches the server directly.
