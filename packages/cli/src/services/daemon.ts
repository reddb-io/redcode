...
import { createRedcodeClient } from "@reddb-io/redcode-sdk/v2/client"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createRedcodeClient>, unknown>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createRedcodeClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createRedcodeClient({ baseUrl: connection.url, headers: connection.headers })
    })
