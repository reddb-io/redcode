...
// test/preload.ts pins REDCODE_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.REDCODE_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.REDCODE_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.REDCODE_MODELS_PATH = undefined
  Flag.REDCODE_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.REDCODE_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.REDCODE_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  test("lists every catalog provider with a curated System One offer", () => {
    ...
    expect(ModelsDev.systemOneOffers(catalog)).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ providerID: "nano-gpt", model: "typesafe/jev-latest" }),
      ]),
    )
  })
