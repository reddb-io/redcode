...
import { ModelsDevPlugin } from "@reddb-io/redcode-core/plugin/models-dev"
    ...
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          ...
          disabled: Flag.REDCODE_DISABLE_MODELS_FETCH,
        }
        Flag.REDCODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.REDCODE_MODELS_PATH = previous.path
          Flag.REDCODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
