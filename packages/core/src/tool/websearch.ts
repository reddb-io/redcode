...
export class ConfigService extends Context.Service<ConfigService, Config>()("@redcode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.REDCODE_WEBSEARCH_PROVIDER === "exa" || process.env.REDCODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.REDCODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("REDCODE_EXPERIMENTAL") || truthy("REDCODE_ENABLE_EXA") || truthy("REDCODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("REDCODE_ENABLE_PARALLEL") || truthy("REDCODE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `redcode/${InstallationVersion}`,
