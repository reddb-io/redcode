...
export const PLAYBOOKS: readonly Playbook[] = [
  ...
  },
  {
    id: "slides",
    use_when:
      "Create a presentation: a deck, talk, pitch or paced walkthrough (a design with target presentation), presented from the review and exported as PDF",
    choose: [
      "Use slides only when the user asks for a deck, presentation, talk or paced walkthrough; reference material, detailed review and dense evidence read better as a scroll page.",
      "Write the story before any markup: the one sentence the audience should leave with, then the slide titles in order. Open with the point, build context, show the evidence, close with the decision or next action.",
      "One idea per slide. When a slide needs two ideas, it is two slides; when it needs a paragraph, the paragraph goes to the speaker notes.",
    ],
    structure: [
      'Create the design with kind "deck" and target presentation. Write every slide as a top-level <section class="slide"> in document order, in one HTML entry; the slide runtime makes each one a screen of the review, numbered in order.',
      "Each slide is a fixed 1920×1080 canvas: design in CSS pixels at that size and never let content leave it. The runtime sizes every section.slide to 1920×1080 with overflow hidden and scales the canvas to fit the review, the presentation window and a standalone HTML export; do not add your own scaling, scrolling or slide navigation.",
      'Give a slide an id (such as id="pricing") when other slides or scenarios refer to it; otherwise it is slide-N. Its first h1, h2 or h3 names it in the review\'s thumbnail strip and on review notes.',
      'Put what you will say in <aside class="notes"> inside the slide. Notes never show on the slide, in the review preview or in the PDF; the presenter view shows them next to the current and next slide.',
      "Mark the rhythm with slide types: a hero slide opens the deck, section slides (one large title on a strong background) separate its parts, content slides carry one point each, and a closing slide states the decision or the ask.",
    ],
    design_rules: [
      "Text density: a title of at most about 8 words and at most 3 to 5 short lines under it (roughly 40 words on the slide). Detail belongs in the speaker notes or an appendix slide.",
      "Type for projection: body text 32px or larger, titles 64px and up, nothing under 24px, including captions, axis labels and footers. The audit flags text under 24px (slide-text-small) and content that leaves the 1920×1080 box (slide-overflow) on every slide.",
      "Contrast for a projector: text contrast at least 4.5:1 (3:1 only for text 48px and up), no thin light-grey type, no meaning carried by colour alone. The audit runs axe on every slide.",
      "Use the project's design system or brand when there is one: its fonts, colours and logo. Keep a consistent grid, margins of at least 96px and one alignment per slide; vary composition between neighbouring slides so the deck does not read as the same card repeated.",
      "Let visuals carry the argument: one chart, diagram, product shot or number per slide, labelled directly. Embed images and fonts locally (design_asset, design_generate); the preview has no network.",
    ],
    pitfalls: [
      "Do not turn every explainer into slides, and do not paste a scroll-page outline into slides without rewriting the narrative.",
      "Do not shrink text to make it fit: cut words or split the slide. Overflowing content is cut off on the projector and in the PDF.",
      "Do not build slide navigation, counters, progress bars or a presenter mode into the deck: the review, the presentation window and the PDF provide them.",
      "Do not put speaker notes, TODOs or source links on the slide face; they go in aside.notes.",
