...
export const PLAYBOOKS: readonly Playbook[] = [
  {
    ...
    structure: [
      '1. Establish the brief: design_document with action="create" or "list"; read .red/DESIGN.md and the Design system block (docs, token files, component roots and their exported components), then action="update" with brief, designSystem, decisions and scenarios. State in designSystem which roots and token files the prototype uses, or that the project has none. Give variant-specific scenarios their variant ID; unscoped scenarios apply to every variant. Write only in the returned root.',
  ...
  },
  {
    ...
    choose: [
      "For existing applications, import their real components from the roots in .red/DESIGN.md and style with the recorded token files; re-implement a component only when the system has none and say so in designSystem. For new interfaces, explore three distinct compositions unless the user supplied a direction.",
    ],
    structure: [
