...
export class Service extends Context.Service<Service, Interface>()("@redcode/ProjectV2") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cached = Effect.fnUntraced(function* (dir: string) {
      for (const name of ["redcode", "opencode"]) {
        const value = yield* fs.readFileString(path.join(dir, name)).pipe(
          Effect.map((value) => value.trim()),
          Effect.catch(() => Effect.succeed(undefined)),
        )
        if (value) return ID.make(value)
      }
    })
    ...
    const commit = Effect.fn("Project.commit")(function* (input: { store: AbsolutePath; id: ID }) {
      yield* fs.writeFileString(path.join(input.store, "redcode"), input.id).pipe(Effect.ignore)
    })
