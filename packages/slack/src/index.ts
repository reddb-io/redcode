...
import { createRedcode, type ToolPart } from "@reddb-io/redcode-sdk"
...
console.log("🚀 Starting Redcode server...")
const redcode = await createRedcode({
  port: 0,
})
console.log("✅ Redcode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await redcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new Redcode session...")
    const { client, server } = redcode
    ...
    console.log("✅ Created Redcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to Redcode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Redcode response:", JSON.stringify(result, null, 2))
