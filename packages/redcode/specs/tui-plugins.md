...
- Author package entrypoint is `@reddb-io/redcode-plugin/tui`.
...
- Import types from `@reddb-io/redcode-plugin/tui`.
- `@reddb-io/redcode-plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@reddb-io/redcode-plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@reddb-io/redcode-plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@reddb-io/redcode-plugin/tui`.
