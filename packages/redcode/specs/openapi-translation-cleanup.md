...
Trim `packages/redcode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/redcode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/redcode`.
- `[x]` `bun typecheck` from `packages/redcode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/redcode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/redcode`.
- `[x]` `bun dev generate > /tmp/opencode-openapi.json` from `packages/redcode`.
...
- Edit `packages/redcode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/opencode-openapi.json` from `packages/redcode`.
...
- Check source schemas in files like `packages/redcode/src/session/schema.ts`, `packages/redcode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/redcode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/redcode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
