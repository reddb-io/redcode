...
Delete `packages/redcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/redcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/redcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/redcode`
- TUI plugin loader tests in `packages/redcode` if runtime plugin API wiring changed
