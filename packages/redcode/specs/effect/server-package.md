...
- The server still lives in `packages/redcode`.
...
Until enough shared service code lives outside `packages/redcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/redcode`

It should not import `packages/redcode` services while `packages/redcode`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/redcode` implementation details.
