...
//   - REDCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - REDCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - REDCODE_DISABLE_PROJECT_CONFIG : skip walking up for opencode.json
//   - REDCODE_PURE                : skip external plugin discovery + install
//   - REDCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
function pickCliCommand(): { command: string; argsPrefix: ReadonlyArray<string> } {
  for (const candidate of cliCandidates) {
    if (existsSync(candidate)) {
      if (candidate.endsWith(".ts")) {
        return { command: "bun", argsPrefix: ["run", candidate] }
      }
  ...
  return { command: "bun", argsPrefix: ["run", cliEntry] }
}
...
export function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    REDCODE_CONFIG_CONTENT: configJson,
  ...
  return Effect.gen(function* () {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          REDCODE_CONFIG_CONTENT: JSON.stringify({
