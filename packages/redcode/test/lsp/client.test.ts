...
describe("LSPClient interop", () => {
  ...
  test("initialize failures include bounded server stderr", async () => {
    const handle = spawnFakeServer({ FAKE_LSP_INITIALIZE_FAILURE: "1" })
    const error = await withTestInstance({
      directory: process.cwd(),
      fn: (ctx) =>
        LSPClient.create({
          serverID: "fake",
    ...
    expect(error).toBeInstanceOf(LSPClient.InitializeError)
    expect(error.stderr).toContain("fake server failed during initialization")
    expect(error.cause?.message).toContain("exited with code 23")
  })
