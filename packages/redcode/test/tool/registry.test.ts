...
describe("tool.registry", () => {
  ...
  it.instance("exposes the task background parameter by default", () =>
    Effect.gen(function* () {
      ...
      expect(task?.description).toContain("Background mode")
    }),
  )

  withoutBackground.instance("hides the task background parameter when the flag turns background subagents off", () =>
    Effect.gen(function* () {
      const registry = yield* ToolRegistry.Service
      const agent = yield* Agent.Service
      const build = yield* agent.get("build")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const configDir = path.join(test.directory, ".opencode")
        const customTools = path.join(configDir, "tools")
        const plugin = path.join(configDir, "node_modules", "@reddb-io", "redcode-plugin")
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@reddb-io/redcode-plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@reddb-io/redcode-plugin"',
              "export default tool({",
