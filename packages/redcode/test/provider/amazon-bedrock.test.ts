...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(deepseek).toMatchObject({ modelId: "deepseek.v3.2" })
      expect(r1).toMatchObject({ modelId: "us.deepseek.r1-v1:0" })
      expect(profile).toMatchObject({ modelId: "us.deepseek.r1-v1:0" })
      expect(arn).toMatchObject({ modelId: "arn:aws:bedrock:us-east-1::foundation-model/deepseek.v3.2" })
    }),
  {
    config: {
      provider: {
