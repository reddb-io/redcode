...
import { PartTable } from "@reddb-io/redcode-core/session/sql"
...
describe("MessageV2.filterCompacted", () => {
  ...
  it.instance("stops loading after finding a retained tail across page boundaries", () =>
    withSession(({ sessionID }) =>
      Effect.gen(function* () {
        const history = yield* fill(sessionID, 120, (index: number) => index + 1)
        const compaction = yield* addUser(sessionID)
        yield* addCompactionPart(sessionID, compaction, history[60])
        const summary = yield* addAssistant(sessionID, compaction, { summary: true, finish: "end_turn" })
        const recent = yield* addUser(sessionID, "recent")

        // This discarded page must never be hydrated by compacted history loading.
        const { db } = yield* Database.Service
        yield* db
          .update(PartTable)
          .set({ data: sql.raw("'{'") as never })
          .where(eq(PartTable.message_id, history[0]))
          .run()
          .pipe(Effect.orDie)
