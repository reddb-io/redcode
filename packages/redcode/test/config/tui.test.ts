...
const cleanState = Effect.gen(function* () {
  ...
  delete process.env.REDCODE_CONFIG
  delete process.env.REDCODE_TUI_CONFIG
...
it.instance("project config takes precedence over REDCODE_TUI_CONFIG (matches REDCODE_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("REDCODE_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from REDCODE_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
