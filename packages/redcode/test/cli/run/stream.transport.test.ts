...
import { RedcodeClient, type GlobalEvent } from "@reddb-io/redcode-sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<RedcodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<RedcodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<RedcodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<RedcodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<RedcodeClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    subscribe?: RedcodeClient["event"]["subscribe"]
    globalEvent?: RedcodeClient["global"]["event"]
    promptAsync?: RedcodeClient["session"]["promptAsync"]
    status?: RedcodeClient["session"]["status"]
    messages?: RedcodeClient["session"]["messages"]
    children?: RedcodeClient["session"]["children"]
    permissions?: RedcodeClient["permission"]["list"]
    questions?: RedcodeClient["question"]["list"]
  } = {},
) {
  const client = new RedcodeClient()

  const subscribe: RedcodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: RedcodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: RedcodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: RedcodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: RedcodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: RedcodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: RedcodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: RedcodeClient["question"]["list"] = input.questions ?? (() => ok([]))
