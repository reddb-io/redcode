...
import { RedcodeClient, type Provider } from "@reddb-io/redcode-sdk/v2"
import type { Resolved } from "@reddb-io/redcode-tui/config"
...
describe("run runtime boot", () => {
  ...
  test("falls back to default tui keymap config when config load fails", async () => {
    ...
    expect(result.keybinds.get("input.newline")?.[0]?.key).toBe("ctrl+return,alt+return,ctrl+j")
    expect(result.keybinds.get("input.steer")?.[0]?.key).toBe("shift+return")
  })
