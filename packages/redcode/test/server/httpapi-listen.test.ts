...
const original = {
  ...
  envPassword: process.env.REDCODE_SERVER_PASSWORD,
  envUsername: process.env.REDCODE_SERVER_USERNAME,
}
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.REDCODE_SERVER_PASSWORD
  else process.env.REDCODE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.REDCODE_SERVER_USERNAME
  else process.env.REDCODE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.REDCODE_SERVER_PASSWORD = auth.password
  Flag.REDCODE_SERVER_USERNAME = auth.username
  process.env.REDCODE_SERVER_PASSWORD = auth.password
  process.env.REDCODE_SERVER_USERNAME = auth.username
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.REDCODE_DISABLE_DEFAULT_PLUGINS
    process.env.REDCODE_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.REDCODE_DISABLE_DEFAULT_PLUGINS
      else process.env.REDCODE_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
