  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("REDCODE_SERVER_PASSWORD", original.envPassword)
        restoreEnv("REDCODE_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            REDCODE_SERVER_PASSWORD: input?.password,
            REDCODE_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
