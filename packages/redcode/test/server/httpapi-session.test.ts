...
import { SessionInputTable, SessionMessageTable, SessionTable } from "@reddb-io/redcode-core/session/sql"
...
const originalWorkspaces = Flag.REDCODE_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.REDCODE_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.REDCODE_EXPERIMENTAL_WORKSPACES = true
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "redcode", "src")
