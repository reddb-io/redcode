...
import { ToolOutputBridge } from "../../src/tool/output-bridge"
...
it.instance("explore agent asks for external directories and allows whitelisted external paths", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("external_directory", ToolOutputBridge.GLOB, explore!.permission).action).toBe("allow")
    expect(
...
it.instance(
  "ToolOutputBridge.GLOB is allowed even when user denies external_directory globally",
  () =>
    Effect.gen(function* () {
      const build = yield* load((svc) => svc.get("build"))
      expect(Permission.evaluate("external_directory", ToolOutputBridge.GLOB, build!.permission).action).toBe("allow")
      expect(Permission.evaluate("external_directory", ToolOutputBridge.DIR, build!.permission).action).toBe("deny")
...
it.instance(
  "ToolOutputBridge.GLOB is allowed even when user denies external_directory per-agent",
  () =>
    Effect.gen(function* () {
      const build = yield* load((svc) => svc.get("build"))
      expect(Permission.evaluate("external_directory", ToolOutputBridge.GLOB, build!.permission).action).toBe("allow")
...
it.instance(
  "explicit ToolOutputBridge.GLOB deny is respected",
  () =>
    Effect.gen(function* () {
      const build = yield* load((svc) => svc.get("build"))
      expect(Permission.evaluate("external_directory", ToolOutputBridge.GLOB, build!.permission).action).toBe("deny")
      expect(Permission.evaluate("external_directory", ToolOutputBridge.DIR, build!.permission).action).toBe("deny")
    }),
  {
    config: {
