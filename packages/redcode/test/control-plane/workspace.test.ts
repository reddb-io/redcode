...
import { EventSequenceTable } from "@reddb-io/redcode-core/event/sql"
...
import { WorkspaceTable } from "@reddb-io/redcode-core/control-plane/workspace.sql"
...
const originalEnv = {
  REDCODE_AUTH_CONTENT: process.env.REDCODE_AUTH_CONTENT,
  REDCODE_EXPERIMENTAL_WORKSPACES: process.env.REDCODE_EXPERIMENTAL_WORKSPACES,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.REDCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.REDCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.REDCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.REDCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
