  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("ShellTool.run")(function* (
      ...
      const publishProgress = Effect.fnUntraced(function* (force = false) {
        ...
        if (!force && published && Date.now() - publishedAt < PROGRESS_INTERVAL_MS) return
      ...
      const code: number | null = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* Effect.forkScoped(
            Effect.sleep(`${PROGRESS_INTERVAL_MS} millis`).pipe(Effect.andThen(publishProgress()), Effect.forever),
          )
