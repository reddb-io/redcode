...
import type { ConsoleState } from "@reddb-io/redcode-core/v1/config/console-state"
...
import { RemoteAuthError } from "@reddb-io/redcode-core/v1/config/error"
...
export class Service extends Context.Service<Service, Interface>()("@redcode/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.REDCODE_CONFIG && !Flag.REDCODE_CONFIG_DIR && !Flag.REDCODE_CONFIG_CONTENT) {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "REDCODE_CONFIG_CONTENT") return "local"
        ...
        if (Flag.REDCODE_CONFIG) {
          yield* merge(Flag.REDCODE_CONFIG, yield* loadFile(Flag.REDCODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.REDCODE_CONFIG })
        }

        if (!Flag.REDCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files(["redcode", "opencode"], ctx.directory, ctx.worktree).pipe(
            Effect.orDie,
          )) {
        ...
        if (Flag.REDCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from REDCODE_CONFIG_DIR", { path: Flag.REDCODE_CONFIG_DIR })
        }
        ...
        if (process.env.REDCODE_CONFIG_CONTENT) {
          const source = "REDCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.REDCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from REDCODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["REDCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("REDCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.REDCODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.REDCODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("REDCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.REDCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.REDCODE_DISABLE_PRUNE) {
