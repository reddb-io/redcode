...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@reddb-io/redcode-plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@reddb-io/redcode-core/effect/app-node-builder"
import { errorData, errorMessage } from "@reddb-io/redcode-tui/util/error"
import { isRecord } from "@reddb-io/redcode-tui/util/record"
...
import { hasTheme, upsertTheme } from "@reddb-io/redcode-tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@reddb-io/redcode-tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@reddb-io/redcode-core/v1/config/plugin"
import { createCommandShim } from "@reddb-io/redcode-tui/plugin/command-shim"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir = [".redcode", ".opencode"].includes(path.basename(source_dir))
      ? path.join(source_dir, "themes")
      : path.join(source_dir, ".redcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".redcode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.REDCODE_PURE ? [] : pluginOrigins
    if (Flag.REDCODE_PURE && pluginOrigins.length) {
    }
