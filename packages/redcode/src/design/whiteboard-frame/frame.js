// Vendored from lavish-axi (https://github.com/kunchenguid/lavish-axi, MIT — see ../vendor/LICENSE.lavish-axi),
// src/whiteboard-frame.js. Edits: the message names, the channel-token global and the body data
// attributes carry the redcode prefix; the asset path is /design/vendor/whiteboard/; the core
// helpers come from ../vendor/whiteboard-core.js. Everything else is as lavish wrote it.
...
// `script/whiteboard-bundle.ts` (Bun) together with Excalidraw, the Mermaid
...
} from "../vendor/whiteboard-core.js";
  ...
  window.top.postMessage(
    {
      ...
      ...(state.channelId
        ...
        : {
            channelToken: String(/** @type {any} */ (window).__redcodeWhiteboardChannelToken || ""),
...
function buildShell(theme, mode) {
  document.body.dataset.redcodeWhiteboardTheme = theme;
  document.body.dataset.redcodeWhiteboardMode = mode;
  ...
  if (mode === "inline") {
    ...
    fullscreenButton.onclick = () => post({ type: "redcode-whiteboard:maximize", diagramIndex: state.diagramIndex });
...
function prepareTeardown(message) {
  ...
  if (!postSave(flushId)) {
    ...
    post({ type: "redcode-whiteboard:teardownReady", flushId });
  }
}
...
function flushSaveNow(message) {
  ...
  if (!postSave(flushId)) {
    ...
    post({ type: "redcode-whiteboard:flushComplete", flushId, ok: true });
  }
}
...
function handleSaveResult(message) {
  ...
  if (flushId === state.teardownFlushId) {
    ...
    post({ type: "redcode-whiteboard:teardownFailed", flushId, error });
    return;
  }
  if (state.flushIds.delete(flushId)) {
    post({ type: "redcode-whiteboard:flushComplete", flushId, ok: Boolean(message.ok) });
  }
}
...
async function queueFeedback() {
  ...
  try {
    ...
    post({
      type: "redcode-whiteboard:queueFeedback",
      diagramIndex: state.diagramIndex,
      diagramId: state.diagramId,
      ...createWhiteboardPersistencePayload(state, scene),
...
function main() {
  // redcode: the bundle and Excalidraw's fonts are served beside the other vendored design assets.
  /** @type {any} */ (window).EXCALIDRAW_ASSET_PATH = `${location.origin}/design/vendor/whiteboard/`;
  ...
  window.addEventListener("message", (event) => {
    ...
    if (msg.type === "redcode-whiteboard:init" && !initialized && typeof msg.channelId === "string" && msg.channelId) {
      initialized = true;
    ...
    if (msg.type === "redcode-whiteboard:sourceChanged") handleSourceChanged(msg);
    if (msg.type === "redcode-whiteboard:prepareTeardown") prepareTeardown(msg);
    if (msg.type === "redcode-whiteboard:flush") flushSaveNow(msg);
    if (msg.type === "redcode-whiteboard:saveResult") handleSaveResult(msg);
    if (msg.type === "redcode-whiteboard:queueResult") {
      resetQueueButton();
      if (msg.ok) {
  ...
  post({ type: "redcode-whiteboard:ready" });
}
