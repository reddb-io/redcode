...
// src/export-bundle.js, unchanged except for the three places marked "redcode:" below: the
// environment-variable fallbacks for the caps are gone (redcode configures them), a file URL whose
// path cannot be decoded is reported as unparseable on every runtime, and the lavish SDK check is
// kept only for artifacts written for lavish. The test suite is vendored beside it.
...
export async function buildSelfContainedHtml(html, options = {}) {
  ...
  const ctx = {
    ...
    // redcode: the caps come from options alone; there is no environment fallback.
    maxAssetBytes: resolveBytes(options.maxAssetBytes, undefined, DEFAULT_MAX_ASSET_BYTES),
    maxBundleBytes: resolveBytes(options.maxBundleBytes, undefined, DEFAULT_MAX_BUNDLE_BYTES),
...
function resolveRef(ref, baseDir, ctx, options = {}) {
  ...
  if (isFileSchemeRef(ref, options)) {
    try {
      // redcode: Bun's fileURLToPath tolerates a malformed percent-encoding that Node rejects;
      // decode the path first so both runtimes report the same unparseable URL.
      const url = new URL(schemeRef.replace(/#.*$/, ""));
      decodeURIComponent(url.pathname);
      const resolved = fileURLToPath(url);
      if (!resolved) return { kind: "unparseable-file-url" };
...
// redcode: our own SDK is injected inline at serve time and never reaches the file on disk, so
// there is nothing of ours to strip; this keeps an artifact written for lavish from phoning home.
