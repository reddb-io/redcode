...
import { FrontmatterError } from "@reddb-io/redcode-core/v1/config/error"
...
const REDCODE_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with Redcode. The model's intuition for what a
// Redcode config should look like is often wrong, and Redcode hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch Redcode's own config files gives it the
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, REDCODE_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@redcode/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_REDCODE_SKILL_NAME] = {
          name: CUSTOMIZE_REDCODE_SKILL_NAME,
          description: CUSTOMIZE_REDCODE_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_REDCODE_SKILL_BODY,
        }
