...
export const RunCommand = effectCmd({
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to REDCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to REDCODE_SERVER_USERNAME or 'redcode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: RedcodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: RedcodeClient, sessionID: string) {
      ...
      async function current(sdk: RedcodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: RedcodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: RedcodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: RedcodeClient) {
        ...
        async function loop(client: RedcodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
