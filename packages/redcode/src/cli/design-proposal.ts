...
export async function run(input: {
  ...
  /** Reads one reply; undefined when the prompt was dismissed. */
  ...
  const outcome = await Effect.runPromise(
    DesignProposal.offer({
      ...
      Effect.catch((error) =>
        Effect.sync(() => {
          input.write(`Could not adopt the detected design system: ${String(error)}`)
...
/** Runs the proposal on the terminal with its own readline; Ctrl+C dismisses it as "not now". */
export async function prompt(directory: string) {
  const lines = readline.createInterface({ input: process.stdin, output: process.stdout, terminal: true })
  const write = (text: string) =>
    process.stdout.write(stripVTControlCharacters(text).replace(/[\x00-\x08\x0b-\x1f\x7f]/g, "") + "\n")
  try {
    return await run({
      directory,
      write,
      ask: (query) =>
        new Promise((resolve) => {
          ...
          lines.question(query, { signal: controller.signal }, (reply) => {
