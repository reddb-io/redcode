...
import { LspEvent } from "@reddb-io/redcode-schema/lsp-event"
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getClients = Effect.fnUntraced(function* (file: string) {
      ...
      const clients = yield* Effect.promise(async () => {
        ...
        async function schedule(server: LSPServer.Info, root: string, key: string) {
          ...
          void handle.process.exited.then((code) => {
            ...
            failed(`Exited with code ${code}${client.stderr ? `: ${client.stderr}` : ""}`)
            void client.shutdown().catch(() => undefined)
            bridge.fork(events.publish(Event.Updated, {}))
          })
          return client
        }
        ...
        for (const server of Object.values(s.servers)) {
          ...
          const selected = await server.root(file, ctx)
          if (!selected) continue
          const root = Filesystem.resolve(selected)
          const key = `${server.id}\0${root}`
          if (s.broken.has(key)) continue
          if (s.unavailable.has(key)) continue
          ...
          if (client || s.broken.has(key)) updated = true
          if (!client) continue
      ...
      if (clients.updated) yield* events.publish(Event.Updated, {})
    ...
    const status = Effect.fn("LSP.status")(function* () {
      ...
      return result.sort((a, b) => a.id.localeCompare(b.id) || a.root.localeCompare(b.root))
    })
    ...
    const hasClients = Effect.fn("LSP.hasClients")(function* (file: string) {
      ...
      return yield* Effect.promise(async () => {
        ...
        for (const server of Object.values(s.servers)) {
          ...
          const root = Filesystem.resolve(selected)
          const key = `${server.id}\0${root}`
          if (s.broken.has(key) || s.unavailable.has(key)) continue
          return true
        }
        return false
      })
...
function lspFailure(error: unknown) {
  const cause = error instanceof LSPClient.InitializeError ? error.cause : error
  const stderr = error instanceof LSPClient.InitializeError ? error.stderr : undefined
  return stripAnsi([errorMessage(cause ?? error), stderr].filter((item): item is string => !!item).join(": "))
    .replace(/[\u0000-\u001f\u007f-\u009f]+/g, " ")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 500)
