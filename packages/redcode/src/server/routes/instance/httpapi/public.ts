...
type OpenApiOperation = {
  ...
  security?: unknown
  "x-codeSamples"?: ReadonlyArray<{ lang: string; source: string }>
}
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for redcode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(RedcodeHttpApi)
...
export const PublicApi = RedcodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "Redcode",
    version: "1.0.0",
    description: "Redcode API",
