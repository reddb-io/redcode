...
`packages/redcode/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/redcode ---\
                      > @reddb-io/redcode-tui -> @reddb-io/redcode-sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/redcode`,
`packages/cli`, or `@reddb-io/redcode-core`.
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/redcode`.
- Do not replace private `packages/redcode` imports with `@reddb-io/redcode-core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@reddb-io/redcode-tui`.

## Ownership Boundary

### `@reddb-io/redcode-tui` Owns
...
```text
packages/redcode/src/cli/cmd/tui
```

Its private dependency on `packages/redcode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/redcode/src/*`.
...
Status: Completed. The private `@reddb-io/redcode-tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@reddb-io/redcode-tui`.
...
- The package has no dependency on `opencode`, `@reddb-io/redcode-cli`, or
...
primitives now live in `@reddb-io/redcode-tui`. The legacy host consumes explicit
...
and build inputs through `@reddb-io/redcode-tui/runtime`. Movable app, component,
...
`@reddb-io/redcode-tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/redcode` initially.
...
canonical in `@reddb-io/redcode-tui`. Configured references resolve through the new
...
`@reddb-io/redcode-tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/redcode`.
...
- The package has no imports from `packages/redcode`, `packages/cli`, or
  `@reddb-io/redcode-core`.
...
public `@reddb-io/redcode-tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/redcode`.
- Keep the legacy embedded worker and server startup in `packages/redcode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@reddb-io/redcode-tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/redcode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@reddb-io/redcode-tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@reddb-io/redcode-core`, or either
  executable package.
...
rg '@reddb-io/redcode-core|packages/redcode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/redcode/src packages/redcode/test
```
...
- Build the current-platform `packages/redcode` binary.
