...
`design.system` is read when a design document is created or refreshed and recorded on it. `paths` are project-relative component and token roots: declaring one is a standing read grant for design builds, asked once at `design_preview` for the roots, the declared stylesheets, the tooling configuration and the project's `node_modules`, so preview builds import from them without a prompt per file. `css` lists stylesheets every preview includes. `tailwind` runs the project's `postcss.config.*` (or tailwindcss with autoprefixer from `tailwind.config.*`), defaulting to on when `tailwind.config.*` exists and tailwindcss is a dependency; because that executes project files in the redcode process, `design_preview` asks a separate `project_tooling` permission for them and builds without the pipeline when refused (Tailwind v4 needs `tailwind: true` explicitly). `framework` defaults from the dependencies; `aliases` add import prefixes on top of the tsconfig paths and take precedence over them. A package linked into `node_modules` from a source tree outside it (a workspace package) is only readable once that tree is declared in `paths`. The `design` block merges key by key, as the legacy configuration's deep merge does: `browser` comes from the most specific configuration document that sets it, and `system` from the most specific one that sets it, together with that document's `application` (system paths are relative to it). A project `design.system` therefore replaces a global one entirely (its arrays are never concatenated) while a global `design.browser` still applies. Because `application` travels with `system`, a project `system` without `application` drops a global `application`, and a project `application` set alone under a global `system` is ignored: set both in the same document. `application` names the package a design targets when `design_document` create names none; a design created for a different, explicitly named application does not get the configured system. `browser` picks the browser for review pages (`default`, an app name or a path); `REDCODE_DESIGN_BROWSER` wins over it.
