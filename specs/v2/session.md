...
- `SessionHistory.systemMessagesAfter` selects admitted system messages after the epoch's `baseline_seq`; V1 lowers each one into legacy history as a synthetic user message wrapped in `<system_update>`, anchored by identity on the user message that started the turn (an update admitted during the turn follows the latest message of that turn created at or before its admission and never precedes that user message), because the AI SDK rejects mid-conversation `system` messages for some providers and merges adjacent user messages.
- `SessionCompaction.process` calls `SessionContextEpoch.requestReplacement` before it writes the summary, so no crash ordering leaves a committed summary under the previous baseline; revert cleanup resets the epoch with the history it removes. A stored snapshot the build cannot decode is replaced by a new epoch once rather than ending the turn.
- Replacement follows `SystemContext.replace` semantics. Legacy compaction leaves no `session_message` compaction row, so the request is durable on the epoch row (`session_context_epoch.replacement_seq`, the sequence at the compaction boundary) and survives a restart. At the next boundary `SessionContextEpoch.prepare` renders a fresh baseline fenced at that sequence, so earlier system updates leave projection; while a previously admitted source is unavailable the replacement is blocked, the stored baseline and snapshot stay in force with their system updates, no ordinary update is admitted, and the turn proceeds. The request stays pending and is retried at every later boundary until it succeeds.

- Prompt inbox: an accepted V1 prompt is an Admitted Prompt. `SessionPrompt.createUserMessage` calls `SessionInput.admit` before it publishes the V1 `message`/`part` rows, so the row is a sidecar next to the canonical V1 message (the TUI shows it as queued) and is model-invisible until Prompt Promotion; the loop hides every user message that still has a pending inbox row. Promotion is the loop re-publishing the stored user message re-stamped to the promotion time (so it takes its place in history after everything the drain wrote since admission; the projector only ever moves `time_created` forward) followed by the durable `message.promoted` event, whose projector stamps `promoted_seq` on the pending row (`SessionInput.projectLegacyPromotion`). No `session.next.prompted` event and no V2 user row are produced, an idempotent `message.updated` retry promotes nothing, and sync/steal replay rebuilds the stamp because it is an aggregate event rather than a commit hook. Surfaces treat a `message.updated` whose id they already hold as a move, not a second entry. `steer` prompts (the default) are promoted together at the next step boundary of the running drain, with the step allowance and the todo-continuation budget reset once for the batch; `queue` prompts are promoted one at a time only where the loop would otherwise go idle. `noReply` is admit-only. A drain that finds the previous turn already finished (a wake, a queued prompt on an idle session) does not review or judge that turn again; it only promotes. Turns ended by the step ceiling, a todo blocker or an orphaned interrupted tool promote at that boundary as well, the Goal already paused or blocked. A removed V1 message (`message.removed`, revert) drops its pending row. A row whose user message is not stored is skipped and the next promotable row is taken; the loop never removes it, because admission and the message rows are separate publications a boundary can fall between and a replica rebuilding its projection sees every row before its message lands, so a removal there would be a durable mistake. The request that admitted the row calls `loop` once its rows are written, and that call is the wake. Compaction summarises and retains only model-visible messages; a pending prompt never reaches the checkpoint. A drain that sends nothing to the provider — a wake, or a compaction-only drain such as `/compact` — runs neither todo review nor the goal judge for the turn it found finished. Cancel leaves pending rows in place: a pending steer stays shown as queued and is delivered at the first boundary of the next drain, ahead of the prompt that starts it (admitted later); queued prompts follow it. `Session.fork` does not copy pending user messages. The V1 run-state runner's pending re-run is the wake coalescer: a run that finds nothing to promote exits at once.

- Managed Tool Output: every text-producing legacy tool (built-in, plugin, MCP tool and MCP resource) bounds its output through core's `ToolOutputStore` via `packages/redcode/src/tool/output-bridge.ts`, which binds the store with limits read per instance from Redcode's `tool_output` config (`ToolOutputStore.layerWith`, `Config.node` an explicit dependency, core defaults outside an instance) and runs the store's hourly retention scan behind a defect guard. Both runtimes therefore share the `tool-output` directory under the data dir, `tool_` file names and the head-and-tail preview: line-count bounding cuts on line boundaries; the byte-bound fallback cuts within a line, as in V2. The store's notice is extended with the legacy instruction to Grep or Read the saved file with offset/limit rather than whole. The shell tool streams into a file the store allocates (`retain`). The legacy read/grep permission for that directory is the same glob V2 whitelists. A storage failure (unwritable directory, full disk, a taken name) yields a lossy bounded preview without a path (`ToolOutputStore.boundedPreview` with a legacy notice, warning logged) instead of a failed tool call, for streamed shell output as well, which the V2 runner does not yet do itself. Retention is inherited unchanged: files are deleted by mtime after seven days regardless of any session that still references them.

Legacy sessions therefore own `session_context_epoch` and `session_message` system rows and `session_input` rows but are never run by `SessionRunner`.
