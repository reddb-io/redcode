...
- Prompt inbox: an accepted V1 prompt is an Admitted Prompt. `SessionPrompt.createUserMessage` calls `SessionInput.admit` before it publishes the V1 `message`/`part` rows, so the row is a sidecar next to the canonical V1 message (the TUI shows it as queued) and is model-invisible until Prompt Promotion; the loop hides every user message that still has a pending inbox row. Promotion is the durable re-publication of the stored user message by the loop itself, with a `commit` hook that stamps `promoted_seq` atomically with the event (`SessionInput.projectLegacyPromotion`), so no `session.next.prompted` event and no V2 user row are produced and an idempotent prompt retry promotes nothing. Commit hooks are not replayed: a rebuilt projection leaves legacy rows pending and the next drain promotes them again. The promoted message is re-stamped with the promotion time, so it takes its place in history after everything the drain wrote since admission. `steer` prompts (the default) are promoted together at the next step boundary of the running drain, with the step allowance and the todo-continuation budget reset once for the batch; `queue` prompts are promoted one at a time only where the loop would otherwise go idle. `noReply` is admit-only. A drain that finds the previous turn already finished (a wake, a queued prompt on an idle session) does not review or judge that turn again; it only promotes. Turns ended by the step ceiling, a todo blocker or an orphaned interrupted tool promote at that boundary as well, the Goal already paused or blocked. A removed V1 message (`message.removed`, revert) drops its pending row, and a row whose message never reached the database is discarded the same way instead of heading the queue. Cancel leaves pending rows in place: steers promote at the first boundary of the next drain, ahead of the prompt that starts it (admitted later), and queued prompts after it. `Session.fork` does not copy pending user messages. The V1 run-state runner's pending re-run is the wake coalescer: a run that finds nothing to promote exits at once.
