...
- `SessionCompaction.process` calls `SessionContextEpoch.reset` before it writes the summary, so no crash ordering leaves a committed summary under the previous baseline; revert cleanup resets the epoch with the history it removes. The next step starts a new epoch fenced after every earlier system message. A stored snapshot the build cannot decode is replaced by a new epoch once rather than ending the turn.
