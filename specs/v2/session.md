...
- Managed Tool Output: every text-producing legacy tool (built-in, plugin, MCP tool and MCP resource) bounds its output through core's `ToolOutputStore` via `packages/redcode/src/tool/output-bridge.ts`, which binds the store with limits read from Redcode's `tool_output` config (`ToolOutputStore.layerWith`) and runs the store's hourly retention scan. Both runtimes therefore share the `tool-output` directory under the data dir, `tool_` file names and the head-and-tail preview with the store's notice; the shell tool streams into a file the store allocates (`retain`). The legacy read/grep permission for that directory is the same glob V2 whitelists. A storage failure yields a lossy bounded preview without a path (`ToolOutputStore.boundedPreview` with a legacy notice, warning logged) instead of a failed tool call, which the V2 runner does not yet do itself.
