...
- Add optional `timing` (`GenerationTiming`, the same contract as the V1 `AssistantMessage.timing`) to the durable event `session.next.step.ended` and to the projected `Session.Message.Assistant`. The V2 runner records it per provider attempt:
  - `requestStarted`: when the provider stream is created, then again before every HTTP attempt through `RequestExecutor.AttemptStarted`, so a status retry's backoff is excluded;
  - `firstToken`, `firstVisible` and `lastToken`: stamped by a reader fiber as events arrive, not when the handler reaches them;
  - `prepMs`: from the start of the attempt to its first request;
  - `outputTokens`, `reasoningTokens` and `reasoningChars`;
  - `idleMs` and `burst`: as in the legacy runtime.
- Tool execution, hooks and the end snapshot, which happen before the event is published, stay outside the window.
- `timing` is written only when the step settles. A V2 step has no live first token, and no surface displays V2 timing yet.
- The message updater assigns the event's `timing` unconditionally, so replaying the durable events rebuilds exactly what was recorded; the TUI's V2 message store does the same.
- Reading ahead of the handler keeps interruption durable: content events (starts, deltas and ends of text, reasoning and tool input) that arrived before an interruption are published before the step closes, up to the first event that would act, such as a tool call.
- Add no durable-event version: the field is optional. `session.next.step.ended.2` events recorded before it decode unchanged, and a reader built from the schema without `timing` decodes a new event and ignores the field (covered in `packages/schema/test/compatibility.test.ts`). Add no route or migration.
