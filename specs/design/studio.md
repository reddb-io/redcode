...
For an existing application, select its directory and reuse its components from a local mounting entry. The entry defaults to `src/main.tsx` and must mount on `#root`. React/Solid resolve from that application's installed dependencies; a linked task worktree without `node_modules` resolves through the checkout it was created from. `compilerOptions.paths` aliases (every target, in order) and the JSX and class-field settings of its `tsconfig.json` are applied. Project Vite configurations and plugins are not executed. Components needing routing, data providers or application services need local fixtures in the mounting entry. HTML uses `index.html` by default.

The `design.system` configuration section declares the project's design system: `paths` (component and token roots), `css` (stylesheets every preview includes: the html engine links a built `design-system.css`, react and solid entries import them first), `tailwind` (run the project's `postcss.config.*`, or tailwindcss with autoprefixer from `tailwind.config.*`, with the prototype added to Tailwind's content; default when `tailwind.config.*` exists and tailwindcss is a dependency), `framework` (default from dependencies) and `aliases`. The effective values are recorded on the document as `system` at creation and refresh. Declaring a root is a standing read grant for design builds only: `design_preview` asks once, in a single prompt, for the declared roots and stylesheets, the tooling configuration (`package.json`, `tsconfig.json`, `postcss.config.*`, `tailwind.config.*`) and the `node_modules` directories bare specifiers resolve through (the application's and the checkout's up to its repository root), then every preview build reads exactly that list without further prompts. Files outside the declared set still prompt per file, a symlink escaping a declared root is refused because the canonical path is checked, and a package linked to a source tree outside those `node_modules` directories (a workspace package, an `npm link`) is not trusted until its source is declared in `paths`; `"."` declares the whole project. Tooling is detected and executed from the checkout that holds `node_modules`, because its configuration requires plugins from there. Enabling Tailwind executes the project's `tailwind.config.*` and `postcss.config.*` in the redcode process, which a read grant does not cover: `design_preview` (and the publish and restore routes) asks a separate `project_tooling` permission naming those files; if it is refused the revision is built without the pipeline, records `system.tailwind: false`, and the tool result says so. An edited configuration applies to the next build without a restart. The decision is recorded on the revision: exports, audits and comparisons re-render a snapshotted revision from that record without asking again, so a grant answered "once" still covers every later re-render of the revisions it produced. Tailwind v4 is best effort: it needs `tailwind: true` explicitly and the prototype directory is not injected as an `@source`, so utilities used only by the prototype may need an `@source` in the project's stylesheet. Sass, less and stylus remain rejected.
