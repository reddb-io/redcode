...
The TUI's legacy session runtime remains distinct from SessionV2. It counts each provider attempt, including retries, skips gates/judging while background work runs, includes recorded tool outputs and real gate results in judging, and rejects completion based on a textual claim alone. Terminal provider failure blocks the Goal. Exhausted resume remains paused until `/goal-budget` increases the budget. Plan uses the same default restrictions and reads/hashes the plan before approval. A Goal started in Plan stays there; the composer changes mode only from explicit authorized transition metadata.
...
| Legacy Goal and Plan runtime                    |            25 | First-attempt budgets, observed evidence, waiting and Plan-only completion                                                   |
