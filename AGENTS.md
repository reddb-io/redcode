...
- Keep delivery vocabulary explicit. Prompts steer by default and promote at the next safe provider-turn boundary while the current drain requires continuation. An explicit `queue` input remains pending until the Session would otherwise become idle; promote one queued input at that boundary, then reevaluate continuation before promoting another. Promoting any new user input resets the selected agent's provider-turn allowance; a batch of steers resets it once. A queued input still pending when the drain it was admitted into ends (an interrupt, a failed turn, a process restart) is stale: it is never promoted at a later idle boundary, only sent explicitly (turned into a steer) or discarded.
