...
const product = await Bun.file(path.join(root, "packages/redcode/package.json")).json()
if (typeof product.version !== "string" || !/^\d+\.\d+\.\d+$/.test(product.version))
  throw new Error("packages/redcode/package.json has no exact semantic version")
